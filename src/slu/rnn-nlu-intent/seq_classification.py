...
def generate_multiple_output(encoder_state, attention_states, sequence_length, targets, num_classes, buckets, 
                       use_mean_attention=False,
                       softmax_loss_function=None, per_example_loss=False, name=None, use_attention=False):
  all_inputs = targets
  with ops.op_scope(all_inputs, name, "model_with_buckets"):
    with variable_scope.variable_scope(variable_scope.get_variable_scope(),
      ...
      loss = tf.reduce_sum(crossent) / math_ops.cast(batch_size, dtypes.float32)
      '''
      loss = tf.reduce_mean = nn_ops.sigmoid_cross_entropy_with_logits(bucket_outputs[0], targets[0])

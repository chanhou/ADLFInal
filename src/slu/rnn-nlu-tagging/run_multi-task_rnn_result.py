...
tf.app.flags.DEFINE_string("model", "1800", "model file.")
...
def create_model(session, source_vocab_size, target_vocab_size, label_vocab_size):
  ...
  # ckpt = FLAGS.train_dir+'/model.ckpt-2100'
  ckpt = FLAGS.train_dir+'/model.ckpt-'+FLAGS.model
  ...
  with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
    ...
    while 1: #model.global_step.eval() < FLAGS.max_training_steps:
      ...
      if 0 == 0:
        ...
        #with open(FLAGS.output+'/intent.txt.'+FLAGS.model,'w')as w:
        #    for intt in intent_list:
        #        w.write(intt+'\n')
        with open(FLAGS.output+'/tagging.txt.'+FLAGS.model,'w')as w:
